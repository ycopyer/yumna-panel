...
const GitManager: React.FC<GitManagerProps> = ({ websiteId, userId, defaultDeployPath }) => {
    ...
    // Log Polling for Git Deploy
    useEffect(() => {
        let pollInterval: any;
        if (installJobId) {
            pollInterval = setInterval(async () => {
                try {
                    const res = await axios.get(`/api/task/${installJobId}`);
                    setInstallLogs(res.data.logs || []);

                    if (res.data.status === 'completed' || res.data.status === 'failed') {
                        clearInterval(pollInterval);
                        setTimeout(() => setInstallJobId(null), 3000);
                        fetchRepos();
                    }
                } catch (e) {
    ...
    const fetchRepos = async () => {
        setLoading(true);
        try {
            const res = await axios.get('/api/git');
    ...
    const handleDeploy = async (id: number) => {
        setActionLoading(id);
        setInstallLogs(['Initiating synchronized deployment...']);
        try {
            const res = await axios.post(`/api/git/${id}/deploy`, {});
            ...
            <AnimatePresence mode="wait">
                {installJobId && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -20 }}
                        className="fixed inset-x-8 bottom-8 z-[150] bg-[#0A0A0A] border border-indigo-500/30 rounded-[32px] p-8 shadow-2xl max-w-4xl mx-auto"
                    >
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 className="text-lg font-black text-white italic uppercase tracking-tight">Deploying via Stream</h4>
                                    <p className="text-[10px] text-indigo-400/60 font-black uppercase tracking-[0.2em]">Real-time synchronization active</p>
                                </div>
                            </div>
                            <button onClick={() => setInstallJobId(null)} className="p-2 hover:bg-white/5 rounded-xl text-white/20 hover:text-white transition-all">
                                <Plus className="rotate-45" size={20} />
                            </button>
                        </div>
                        <div className="bg-black/40 rounded-2xl p-6 h-64 overflow-y-auto font-mono text-xs text-white/60 space-y-1.5 custom-scrollbar border border-white/5">
                            {installLogs.map((log, i) => (
                                <div key={i} className="flex gap-4">
                                    <span className="text-white/10 shrink-0">[{i + 1}]</span>
                                    <span className={log.includes('failed') || log.includes('error') ? 'text-rose-400' : 'text-indigo-300'}>{log}</span>
                                </div>
                            ))}
                        </div>
                    </motion.div>
                                            ...
                                            {repo.webhookSecret && (
                                                <div className="mt-4 p-4 bg-black/20 rounded-2xl border border-white/5 space-y-3">
                                                    ...
                                                    <div className="grid gap-2">
                                                        <div className="flex items-center gap-2 bg-white/5 p-2 rounded-lg group/url hover:bg-white/10 transition-colors cursor-pointer" onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/api/git/webhook/${repo.id}`); alert('Webhook URL Copied'); }}>
                                                            <code className="text-[10px] text-white/60 font-mono truncate flex-1">
                                                                {window.location.origin}/api/git/webhook/{repo.id}
                                                            </code>
...
const DeployKeySection: React.FC<{ userId: number }> = ({ userId }) => {
    ...
    const fetchKey = async () => {
        try {
            const res = await axios.get('/api/git/key');
    ...
    const handleRegen = async () => {
        ...
        setLoading(true);
        try {
            // Note: In v3 we don't have a regen endpoint yet, but we can reuse the same /key logic 
            // if we add a flag. For now, let's keep it simple.
            const res = await axios.get('/api/git/key?regen=true');
            setKey(res.data.publicKey);
        } catch (e) {
