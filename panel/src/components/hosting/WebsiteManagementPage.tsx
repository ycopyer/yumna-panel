...
const WebsiteManagementPage: React.FC<WebsiteManagementPageProps> = ({
    ...
    useEffect(() => {
        let interval: any;
        if (installJobId && installStatus === 'running') {
            interval = setInterval(async () => {
                try {
                    const res = await axios.get(`/api/websites/${website.id}/install/logs`, {
                        params: { jobId: installJobId }
                    });
                    setInstallLogs(res.data.logs || []);
                    setInstallStatus(res.data.status);
                    if (res.data.status !== 'running') {
                        clearInterval(interval);
                    }
                } catch (err) {
                    console.error('Failed to fetch install logs', err);
                }
            }, 2000);
        }
        return () => clearInterval(interval);
    }, [installJobId, installStatus]);
                                    ...
                                    {[
                                        ...
                                        <div key={app.id} className="bg-white/[0.02] border border-white/5 rounded-[48px] p-10 hover:bg-white/[0.04] transition-all group overflow-hidden relative">
                                            <div className="relative z-10 space-y-6">
                                                ...
                                                <button
                                                    onClick={async () => {
                                                        ...
                                                        if (confirm(`Begin ${app.name} installation?`)) {
                                                            ...
                                                            try {
                                                                const res = await axios.post(`/api/websites/${website.id}/install`, { appType: app.name });
                                                                setInstallJobId(res.data.jobId);
                                                            } catch (err: any) {
                                                                alert(err.response?.data?.error || 'Failed to start installation');
                                                                setActionLoading(null);
                                                            }
                                                        }
                                                    }}
                                                ...
                                                >
                                                    {actionLoading === `installing-${app.id}` ? 'Deploying...' : app.coming ? 'Coming Soon' : `Deploy ${app.name} Node`}
                                                </button>
                                            </div>
                                ...
                                {installJobId && (
                                    ...
                                    >
                                        <motion.div
                                            ...
                                            animate={{ scale: 1, y: 0 }}
                                            className="w-full max-w-4xl bg-[#0a0f1d] border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl flex flex-col max-h-[80vh]"
                                        >
                                            <div className="px-10 py-8 border-b border-white/5 flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 bg-indigo-500/10 rounded-2xl flex items-center justify-center text-indigo-500">
                                                        <Rocket size={24} className={installStatus === 'running' ? 'animate-bounce' : ''} />
                                                    </div>
                                                    <div>
                                                        <h3 className="text-xl font-black text-white tracking-tight">Node Deployment Sequence</h3>
                                                        <p className="text-[10px] font-black text-white/20 uppercase tracking-widest mt-1">Status: {installStatus}</p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="flex-1 p-8 bg-black/40 overflow-auto custom-scrollbar font-mono text-[11px] leading-relaxed text-indigo-400">
                                                {installLogs.length === 0 ? (
                                                    <div className="flex flex-col items-center justify-center py-20 gap-4 opacity-20">
                                                        <Loader2 size={32} className="animate-spin" />
                                                        <p className="uppercase font-black tracking-[0.3em]">Establishing secure uplink...</p>
                                                    </div>
                                                ) : (
                                                    installLogs.map((log, i) => (
                                                        <div key={i} className="mb-1 border-l-2 border-white/5 pl-4 hover:border-indigo-500/30 transition-colors">
                                                            {log}
                                                        </div>
                                                    ))
                                                )}
                                                {installStatus === 'running' && (
                                                    <div className="mt-4 flex items-center gap-3 text-white/20 italic animate-pulse">
                                                        <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full" />
                                                        Awaiting telemetric data...
                                                    </div>
                                                )}
                                            </div>
                                            <div className="px-10 py-6 bg-white/[0.02] border-t border-white/5 flex items-center justify-between">
                                                <div className="flex gap-2">
                                                    <div className={`w-2 h-2 rounded-full ${installStatus === 'running' ? 'bg-indigo-500 animate-pulse' : installStatus === 'completed' ? 'bg-emerald-500' : 'bg-rose-500'}`} />
                                                    <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">System Check: {installStatus.toUpperCase()}</span>
                                                </div>
                                                {installStatus === 'completed' && (
                                                    <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Deployment Successful • Node Online</span>
                                                )}
                                            </div>
                                        </motion.div>
                                    </motion.div>
