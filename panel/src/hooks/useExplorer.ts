...
export const useExplorer = (user: any, onLogout: () => void) => {
    ...
    useEffect(() => {
        ...
        else if (['nodes', 'collaboration', 'security-center', 'security-dashboard', 'fail2ban', 'waf', 'malware', 'integrity', 'vulnerability', '2fa', 'monitor', 'audit', 'cron', 'ssl', 'ftp', 'apps', 'domains', 'docker', 'billing', 'admin-billing', 'fraud', 'cloud'].includes(activeView as string)) {
