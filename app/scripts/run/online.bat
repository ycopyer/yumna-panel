...
if "%AUTO_SKIP%"=="Y" (
    echo [INFO] Existing installation detected. Starting services...
    
    :: Start MariaDB
    echo [1/3] Restarting MariaDB...
    taskkill /IM mysqld.exe /F >nul 2>nul
    start /B "" "%PANEL_ROOT%\bin\database\mariadb\bin\mysqld.exe" --datadir="%PANEL_ROOT%\data\mysql"
    
    :: Start PM2 Panel
    echo [2/3] Starting Yumna Panel...
    cd /d "%PANEL_ROOT%\app"
    ...
    if exist "%PANEL_ROOT%\bin\web\nginx\nginx.exe" (
        ...
        start /B "" "%PANEL_ROOT%\bin\web\apache\bin\httpd.exe"
    )
...
if "%DB_CHOICE%"=="1" (
    echo [INFO] Restarting MariaDB Native Service...
    taskkill /IM mysqld.exe /F >nul 2>nul
    start /B "" "%PANEL_ROOT%\bin\database\mariadb\bin\mysqld.exe" --datadir="%PANEL_ROOT%\data\mysql"
...
echo ====================================================
if "%HIDDEN_MODE%"=="Y" (
    echo [INFO] Running in background. Exiting launcher...
    timeout /t 3 >nul
) else (
    pause
)
